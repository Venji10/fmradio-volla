...
int COM_get_ps(int fd, RDSData_Struct *rds, uint8_t **ps, int *ps_len)
{
    ...
    if (rds->event_status&RDS_EVENT_PROGRAMNAME) {
        printf("%s, Success,[event_status=%d]\n", __func__, rds->event_status);
        ...
        *ps_len = sizeof(rds->PS_Data.PS[3]);
        ...
        printf("%s, Failed,[event_status=%d]\n", __func__, rds->event_status);
        *ps = NULL;
        *ps_len = 0;
        ret = -ERR_RDS_NO_DATA;
    }
...
int COM_get_pi(int fd, RDSData_Struct *rds, uint16_t *pi)
{
    ...
    if (rds->event_status & RDS_EVENT_PI_CODE) {
        printf("%s, Success,[event_status=%d] [PI=%d]\n", __func__, rds->event_status, rds->PI);
        *pi = rds->PI;
    } else {
        printf("%s, Failed, there's no pi,[event_status=%d]\n", __func__, rds->event_status);
        *pi = -1;
        ret = -ERR_RDS_NO_DATA;
    }
...
int COM_get_ecc(int fd, RDSData_Struct *rds, uint8_t *ecc)
{
    ...
    if (rds->event_status & RDS_EVENT_ECC_CODE) {
        printf("%s, Success,[event_status=%d] [ECC=%d]\n", __func__,
             rds->event_status, rds->Extend_Country_Code);
        *ecc = rds->Extend_Country_Code;
    } else {
        printf("%s, Failed, there's no ECC,[event_status=%d]\n", __func__, rds->event_status);
        *ecc = -1;
        ret = -ERR_RDS_NO_DATA;
    }
...
int COM_get_pty(int fd, RDSData_Struct *rds, uint8_t *pty)
{
    ...
    if(rds->event_status&RDS_EVENT_PTY_CODE){
        printf("%s, Success,[event_status=%d] [PTY=%d]\n", __func__, rds->event_status, rds->PTY);
        *pty = rds->PTY;
    }else{
        printf("%s, Success, there's no pty,[event_status=%d]\n", __func__, rds->event_status);
        *pty = -1;
        ret = -ERR_RDS_NO_DATA;
    }
...
int COM_is_rdsrx_support(int fd, int *supt)
{
    ...
    if (fd < 0) {
        printf("FM isRDSsupport fail, g_fm_fd = %d\n", fd);
        *supt = -1;
        ret = -ERR_INVALID_FD;
        return ret;
    }
    ...
    if (ret) {
        printf("FM FM_IOCTL_RDS_SUPPORT fail, errno = %d\n", errno);
        //don't support
        *supt = 0;
        return ret;
    }
    printf("isRDSsupport Success,[support=%d]\n", support);
    *supt = support;
    return ret;
}
...
int COM_Soft_Mute_Tune(int fd, fm_softmute_tune_t *para)
{
    ...
    if (ret) {
        printf("FM soft mute tune faild:%d\n",ret);
        return ret;
    }
#if 0
    printf("Raw data of soft mute tune[%d]: RSSI:[%x]PAMD:[%x]MR:[%x]ATDC:[%x]PRX:[%x]SMG:[%x]",para->freq,value.RSSI,value.PAMD,value.MR,value.ATDC,value.PRX,value.SMG);
    ...
    //printf("soft mute tune[%d] valid[%d]: RSSI:[%d]PAMD:[%d]MR:[%d]ATDC:[%d]PRX:[%d]SMG:[%d]",para->freq,para->valid,RSSI,PAMD,MR,ATDC,PRX,softmuteGainLvl);
    return 0;
}
...
int COM_turn_on_off_rds(int fd, int onoff)
{
    ...
    if (onoff == FMR_RDS_ON) {
        ...
        printf("Rdsset Success,[rds_on=%d]\n", rds_on);
    } else {
        rds_on = 0;
        ret = ioctl(fd, FM_IOCTL_RDS_ONOFF, &rds_on);
        if (ret) {
...
static int COM_get_af_pi(int fd, uint16_t *pi)
{
    ...
    if (ret) {
        printf("COM_get_af_pi fail(%d)\n", ret);
        *pi = 0;
        return ret;
    }
    if (rrd.len == 0) {
        printf("COM_get_af_pi fail, RDS log empty!\n");
        *pi = 0;
        return -1;
    }
    pi1 = rrd.data[4];
...
int COM_active_af(int fd, RDSData_Struct *rds, CUST_cfg_ds *cfg_data, uint16_t orig_pi, uint16_t cur_freq, uint16_t *ret_freq)
{
    ...
    for (i = 0; i < 3 && (PAMD_Value < AF_PAMD_LBound); i++) {
        usleep(10 * 1000);
        ioctl(fd, FM_IOCTL_GETCURPAMD, &PAMD_Value);
        printf("check PAMD %d time(s), PAMD =%d", i+1, PAMD_Value);
    }
    ...
    if (PAMD_Value < AF_PAMD_LBound) {
        ...
        for (i = 0, j = 0; i < af_list_backup.AF_Num; i++) {
            ...
            printf("af list pre-check:freq %d, valid:%d\n", smt_parm.freq, smt_parm.valid);
            if(smt_parm.valid == 1) {
        ...
        for (i = 0; i < af_list.AF_Num; i++) {
            ...
            if (set_freq != org_freq) {
                ...
                if (PAMD_Level[i] < AF_PAMD_HBound) {
                    printf("PAMD_Level[%d] =%d < AF_PAMD_HBound, continue", i, PAMD_Level[i]);
                    continue;
                }
                ...
                if (orig_pi != PI[i]) {
                    printf("pi not match, current pi(%04x), orig pi(%04x)\n", PI[i], orig_pi);
                    continue;
                }
                ...
                if (PAMD_Level[i] > AF_PAMD_HBound) {
                    printf("PAMD_Level[%d] =%d > AF_PAMD_HBound, af switch", i, PAMD_Level[i]);
                    sw_freq = set_freq;
        ...
        printf("AF deside tune to freq: %d, PAMD_Level: %d\n", sw_freq, PAMD_Value);
    ...
    if(rds->event_status&RDS_EVENT_TAON){
        ...
        *backup_freq = org_freq;
        ...
        for(i=0; i< rds->AFON_Data.AF_Num; i++){
            ...
            printf("set_freq=0x%02x,org_freq=0x%02x\n", set_freq, org_freq);
            if(set_freq != org_freq){
                parm.freq = sw_freq;
                ioctl(fd, FM_IOCTL_TUNE, &parm);
                ioctl(fd, FM_IOCTL_GETCURPAMD, &PAMD_Level[i]);
